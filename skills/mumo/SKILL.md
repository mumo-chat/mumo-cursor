...
**Start with a round.** Call `create_deliberation` and pass `application: "Cursor"`. The first response contains each model's prose plus a cross-model claim map showing where the panel agrees and where it splits.
