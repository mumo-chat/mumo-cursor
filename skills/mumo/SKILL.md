...
1. Call `create_deliberation` with the user's problem. Set `application: "Cursor"`.
...
Pass `moderator_name` on `create_deliberation` set to your own model identity (e.g., "Claude Opus 4.6", "GPT-5.5", "Gemini 3.1 Pro"). The audit trail should reflect who's steering. Don't use the user's name; their identity is already on the session.
