...
1. Call `create_deliberation` with the user's problem. Set `application` to `"Cursor"`. Set `moderator_name` to your own model identity (e.g., "Claude Opus 4.6", "GPT-5.5", or "Gemini 3.1 Pro") for audit clarity — not the user's name; their identity is already on the session. Optionally set `takeaway: true` for a structured round 0 Takeaway — see [Takeaway](#takeaway-opt-in).
